...
int x;
int y;
}enemy[99]; //this declares a maximum of 100 enemies, from 0 to 99
...
void Print_Instructions() {
    ...
    cout<<"BUT BEWARE! Enemies, indicated by the number 2 will try to stop you"<<endl;
    ...
    cout<<"A tip to consider: Enemies may be hiding behind the treasure!"<<endl;
}

void InitializeBoardStandard(int board [100][100], customboard &custom, AI enemy[99])
{
...
void RandomPlaceEnemies(int board[100][100], customboard &custom, AI enemy[99]) {
    int enemyID;
...
void RunGame(int board[100][100], AI enemy[99], customboard &custom) {
    ...
    cout<<"Move:  ";
    string avatarMove;
    cin>>avatarMove; //to select your move

    if (avatary==0 && avatarMove=="w") {
        avatarMove="s"; //keeps the avatar on the board if he tries to go too high
    }
    if (avatary==(custom.height-1) && avatarMove=="s") {
        avatarMove="w"; //keeps the avatar on the board if he tries to go too low
    }
    if (avatarx==0 && avatarMove=="a") {
        avatarMove="d"; //keeps the avatar on the board if he tries to go too far right
    }
    if (avatarx==(custom.width-1) && avatarMove=="d") {
        avatarMove="a"; //keeps the avatar on the board if he tries to go too far left
    }
    ...
    if (avatarMove != "w" && avatarMove != "s" && avatarMove != "a" && avatarMove != "d") {
        cout<<"Incorrect Input"<<endl;
        cout<<"Enter one of the following values:"<<endl;
        cout<<"w, s, a, or d" <<endl;
    ...
    if (enemy[enemyID].y==0 && Enemy_Move==4) {
        Enemy_Move=1; //keeps the enemy on the board if he tries to go too high
    }
    if (enemy[enemyID].y==custom.height-1 && Enemy_Move==1) {
        Enemy_Move=4; //keeps the enemy on the board if he tries to go too low
    }
    if (enemy[enemyID].x==0 && Enemy_Move==2) {
        Enemy_Move=3; //keeps the enemy on the board if he tries to go too far right
    }
    if (enemy[enemyID].x==custom.width-1 && Enemy_Move==3) {
        Enemy_Move=2; //keeps the enemy on the board if he tries to go too far left
    }
    ...
    //The previous four if statements force the enemy to attack the avatar if possible
     ...
     if ( avatarx==enemy[enemyID].x && avatary==enemy[enemyID].y ) {
        ...
        return; //leave the game if you lost
    }
...
void RunCampaign(int board[100][100], AI enemy[99], customboard &custom, constants level[9])
{
    level[0].height=4;
    level[0].width=6;
    level[0].enemies=2;
    ...
    int difficulty;
    cout<<"\n\nEnter 1 for easy difficulty"<<endl;
    cout<<"Enter 2 for medium difficulty"<<endl;
    cout<<"Enter 3 for hard difficulty"<<endl;
    cout<<"Selection: ";
    cin>>difficulty;
...
default:
    ...
    for (levelnumber=0; levelnumber<10; levelnumber++) { //cycle through levels
        ...
        RandomPlaceEnemies(board, custom, enemy);
        RunGame(board, enemy, custom); //where the game is actually run
        ...
        if (custom.winloss==1 && levelnumber==9 && difficulty!=3) { //if you beat all 10 levels
        ...
        if (custom.winloss==1 && levelnumber==9 && difficulty==3) {
            cout<<"You have beaten the game\n\n"<<endl;
            cout<<"Credits: "<<endl;
            cout<<"Drew Loughran -- Level Design Assistant"<<endl;
            cout<<"Kevin Loughran -- Debugging Department Head"<<endl;
            cout<<"\nBrian Loughran -- Head Designer"<<endl;
            return;
        }
    }
}
...
int main() {
    int board[100][100]; //initializes a board of maximum size 101x101
    int selection;
    while (selection!=5) { //keeps the menu open if you dont exit
    ...
    switch (selection) {
case 1:
    InitializeBoardStandard(board, custom, enemy);
    RunGame(board, enemy, custom);
    break;
case 2:
    ...
    RandomPlaceEnemies(board, custom, enemy);
    RunGame(board, enemy, custom); //This allows for board customization, if I ever write the code for it
    break;
case 3:
    Print_Instructions(); //prints out some instructions
    break;
case 4:
    RunCampaign(board, enemy, custom, level); //a series of levels to complete with varying difficulty
case 5:
